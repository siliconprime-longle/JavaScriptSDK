describe("ACL", function () {
     ...
     it("Should update the ACL object.", function (done) {
        ...
        obj.save().then(function(list) {
            list.ACL.setUserWriteAccess("y",true);
            list.save().then(function(obj) {
                var query = new CB.CloudQuery();
                query.findById(obj.id, {
                    success : function(obj){
                        if(acl.write.allow.user.length === 2) {
                            done();
                        }
                        else
                            done("Cannot update the ACL");
                    }, error : function(error) {
                        done("Cannot retrieve a saved CloudObject");
                    }
                })
            }, function(error){
                done("ACL cannot be updated");
            });
